...
export const ImageOptions = (attributes) => {
	...
	if (!hideImagePicker && !imageUrl[breakpoints[0]]) {
		return (
			<UseToggle {...generateUseToggleConfig(attributes, manifest, 'imageUse')}>
				<Control icon={icons.image} label={__('Image', 'testinggg')} additionalLabelClasses='es-h-spaced' noBottomSpacing>
					<MediaPlaceholder
						labels={{
							title: __('Add an image', 'testinggg'),
	...
	return (
		<UseToggle {...generateUseToggleConfig(attributes, manifest, 'imageUse')}>
			{!hideImagePicker &&
				<Responsive label={__('Image', 'testinggg')} icon={icons.image}>
						...
						return (
							<div className='es-h-center es-items-end! es-gap-0!' key={breakpointName}>
								...
								<Button
									key={index}
									icon={icons.trashAlt}
									label={__('Remove image', 'testinggg')}
			...
			<Section
				...
				icon={icons.design}
				label={__('Design & layout', 'testinggg')}
			...
			>
				...
				{!hideFullSizeToggle &&
					<IconToggle
						icon={icons.expandXl}
						label={__('Fill container', 'testinggg')}
						checked={imageFull}
			...
			<Section
				showIf={!hideAltText}
				icon={icons.a11y}
				label={__('Accessibility', 'testinggg')}
				noBottomSpacing={!additionalControlsAfterA11y}
			>
				<TextControl
					label={<IconLabel icon={icons.altText} label={__('Alt text', 'testinggg')} />}
					value={imageAlt}
					onChange={(value) => setAttributes({ [getAttrKey('imageAlt', attributes, manifest)]: value })}
					help={__('Describes the content of the image', 'testinggg')}
