...
export const ButtonOptions = (attributes) => {
	...
	return (
		<UseToggle {...generateUseToggleConfig(attributes, manifest, 'buttonUse')}>
			...
			<Section showIf={!hideAriaLabel} icon={icons.a11y} label={__('Accessibility', 'testinggg')} collapsable reducedBottomSpacing>
				<TextControl
					label={<IconLabel icon={icons.ariaLabel} label={__('ARIA label', 'testinggg')} />}
					value={buttonAriaLabel}
					onChange={(value) => setAttributes({ [getAttrKey('buttonAriaLabel', attributes, manifest)]: value })}
					help={__('Description of the button.', 'testinggg')}
				/>
			</Section>

			<Section showIf={!hideId} icon={icons.tools} label={__('Advanced', 'testinggg')} collapsable noBottomSpacing>
				<TextControl
					label={<IconLabel icon={icons.id} label={__('Unique identifier', 'testinggg')} />}
					value={buttonId}
