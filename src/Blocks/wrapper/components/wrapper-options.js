...
export const WrapperOptions = ({ attributes, setAttributes }) => {
	const {
		label = __('Spacing & layout', 'testinggg'),
		showWrapperId = true,
	...
	let wrapperUseOptions = [
		{
			label: __('Just the block', 'testinggg'),
			value: 'off',
	...
	if (wrapperSimpleShowControl) {
		wrapperUseOptions = [...wrapperUseOptions, {
			label: __('Spacing only', 'testinggg'),
			value: 'simple',
					...
					{!wrapperNoControls && wrapperUseShowControl &&
						<div className='es-display-flex es-items-center es-gap-1 es-ml-auto es-flex-shrink-0 es-mr-1 es-wrapper-options-copy-paste'>
							<Button
								...
								label={__('Copy configuration', 'testinggg')}
								showTooltip
							/>
							...
							<Button
								...
								className='es-button-icon-24 es-button-square-24 es-rounded-0.75!'
								label={__('Paste configuration', 'testinggg')}
								showTooltip
							/>
						</div>
					}
			...
			{wrapperUse &&
				<>
					...
					{wrapperUse && showWrapperTag &&
						<OptionSelector
							label={__('Wrapper tag', 'testinggg')}
							icon={icons.code}
					...
					>
						...
						{(!noLeftSpacingInSelector || !noRightSpacingSelector) &&
							<Collapsable label={__('More', 'testinggg')} icon={icons.moreH}>
								{!noLeftSpacingSelector &&
									<ResponsiveNumberPicker
										icon={icons.spacingLeft}
										label={__('Left', 'testinggg')}
						...
						{showWrapperSpacingBottomIn &&
							<ResponsiveNumberPicker
								...
								label={__('Bottom inner', 'testinggg')}
						...
						{(!noLeftSpacingInSelector || !noRightSpacingInSelector) &&
							<Collapsable label={__('More', 'testinggg')} icon={icons.moreH} noBottomSpacing>
								{!noLeftSpacingInSelector &&
									<ResponsiveNumberPicker
										icon={icons.spacingLeftIn}
										label={__('Left inner', 'testinggg')}
								...
								{!noRightSpacingInSelector &&
									<ResponsiveNumberPicker
										...
										label={__('Right inner', 'testinggg')}
					...
					>
						{showWrapperBgColorPicker &&
							<AdvancedColorPicker
								...
								types={
									[
										{
											label: __('None', 'testinggg'),
											value: '',
											icon: icons.emptyCircle,
										},
										{
											label: __('Project color', 'testinggg'),
											value: 'project',
											icon: icons.colorAlt,
										},
										{
											label: __('Gradient', 'testinggg'),
											value: 'gradient',
											icon: icons.gradient,
										}
									]
						...
						{showWrapperRoundedCorners &&
							<ResponsiveNumberPicker
								...
								label={__('Rounded corners', 'testinggg')}
						...
						{(showWrapperDividerTop || showWrapperDividerBottom || showWrapperDividerLeft || showWrapperDividerRight || showWrapperDividerColor) &&
							...
							>
								{showWrapperDividerColor &&
									<ColorPicker
										colors={[{ name: __('None', 'testinggg'), slug: undefined, color: 'transparent' }, ...dividerColors]}
										...
										pickerPopupTitle={__('Divider color', 'testinggg')}
										label={__('Color', 'testinggg')}
										icon={icons.colorAlt}
									/>
								}
											...
											{breakpointNames.map((breakpoint, index) => {
												...
												return (
													<Button
														...
														label={attributeValue ? __('On', 'testinggg') : __('Off', 'testinggg')}
														showTooltip
													/>
												);
											})}
					...
					<Section
						...
						icon={icons.tools}
						label={__('Advanced', 'testinggg')}
						subtitle={__('Visibility, anchor, ID', 'testinggg')}
						noBottomSpacing
						collapsable
					>
						{showWrapperHide &&
						...
						{showWrapperAnchorId &&
							<TextControl
								label={<IconLabel icon={icons.anchor} label={__('Block Anchor ID', 'testinggg')} />}
						...
						{showWrapperId &&
							<TextControl
								label={<IconLabel icon={icons.id} label={__('Block unique identifier', 'testinggg')} />}
								value={wrapperId}
