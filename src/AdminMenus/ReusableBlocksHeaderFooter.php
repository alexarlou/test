 ...
 *
 * @package Testinggg\AdminMenus
 */

declare(strict_types=1);

namespace Testinggg\AdminMenus;

use TestingggVendor\EightshiftLibs\AdminMenus\AbstractAdminMenu;
...
class ReusableBlocksHeaderFooter extends AbstractAdminMenu
{
	...
	{
		return \esc_html__('Header & footer', 'testinggg');
	}
	...
	public function renderPartialSelector($args): void
	{
		...
		if ($reusableBlocksQuery->have_posts()) {
			...
			<select id="<?php echo \esc_attr($args['label_for']); ?>" name="<?php echo \esc_attr($type); ?>">
				<option value="">
					&mdash; <?php \esc_html_e('None', 'testinggg'); ?> &mdash;
				</option>
		...
		<?php } else { ?>
			<i><?php echo \esc_html__('No reusable blocks found.', 'testinggg'); ?></i>
			<?php
		}
	}
